...
void LimitOrderBook::applyEvent(const MarketDataEvent &event) {
  // Dispatch the event to a handler based on its action field.
  // Cases follow XEUR.EOBI frequency: A and C dominate the stream (~50/50).
  switch (event.action) {
  case 'A':
    onAdd(event);
    break;
  ...
  case 'F':
    ...
    break;
  default:
    break; // action='N': no operation
  }
}

void LimitOrderBook::onAdd(const MarketDataEvent &e) {
  // A new limit order was submitted to the exchange. Insert it into the
  // appropriate side of the book at its price.
  if (!e.price.has_value() || e.size <= 0)
    return; // reject invalid data
  const double px = e.priceAsDouble();
  // Index the order ID for fast cancellation.
...
void LimitOrderBook::onCancel(const MarketDataEvent &e) {
  // A participant withdrew an order. Locate it through the index and remove
  // it from the corresponding price level.
  ...
  if (side == 'B') {
    ...
    if (lvl != bids_.end()) {
      ...
      if (lvl->second.empty())
        bids_.erase(lvl); // remove empty level
    }
  } else {
...
void LimitOrderBook::printSnapshot(std::ostream &os, int depth) const {
  os << "  --- ASK ---\n";
  // Print asks in reverse so the least competitive visible level is on top.
  int cnt = 0;
  ...
  for (auto it = askLevels.rbegin(); it != askLevels.rend(); ++it) {
    long long vol = 0;
    for (auto &[oid, sz] : (*it)->second)
      vol += sz;
    os << std::fixed << std::setprecision(6) << "  " << (*it)->first << "  x"
       << vol << "\n";
  }
  os << "  -----------\n";
  cnt = 0;
