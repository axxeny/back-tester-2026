...
| Consume book update, snapshot, and trade messages | One atomic `MarketDelivery` may carry a top-N book view, snapshot marker, and ordered trade span | `src/core/Events.hpp`, `src/runtime/BacktestRuntime.cpp` | `test/CoreContractsTest.cpp`, `test/RuntimeTest.cpp`, Python callback tests |
...
| `HistoricalLOB + EngineView = SimulatedLOB` | Shared historical L3 store plus `SimulatedLOB`-owned private orders and revision-keyed consumption | `src/market/`, `src/trading/SimulatedLOB.*` | `test/CoreMarketTest.cpp`, `test/TradingTest.cpp`, `test/TypedSimulatedLOBTest.cpp` |
| Fill-at-touch matching | Marketable private limits sweep displayed historical orders price-by-price up to the limit, with partial fills | `src/trading/SimulatedLOB.cpp`, `src/market/LimitOrderBook.*` | trading sweep, limit-protection, and partial-fill tests |
| Resting order matching | Own remainders rest in `EngineView` price-time indexes and are reevaluated after later market deliveries | `src/trading/SimulatedLOB.*` | resting-fill and FIFO tests in `test/TradingTest.cpp` |
...
| Synthetic matching harness | Native tests cover touch, partial, multi-level, limit, resting, FIFO, revisions, rejects, and isolation | `test/TradingTest.cpp`, `test/TypedSimulatedLOBTest.cpp` | `uv run ctest --test-dir build-release --output-on-failure` |
...
| N concurrent EngineViews bonus | Not exposed by the production runtime; independent typed `SimulatedLOB` instances preserve isolated private consumption as an extension proof | `src/trading/SimulatedLOB.*` | `test/TypedSimulatedLOBTest.cpp` |
...
| Slippage Simulator | No separate authority; fixed latency plus optimistic displayed-depth matching defines execution | scope and matching architecture |
