...
TEST_CASE("ShardedBookDispatcher routes unique missing-instrument events",
          "[lob][dispatcher][sharded]") {
  ShardedBookDispatcher dispatcher(
      DispatchOptions{.snapshot_every = 2, .max_snapshots = 2, .snapshot_depth = 2},
      ShardedDispatchOptions{.worker_count = 2, .queue_capacity = 4});
  ...
  dispatcher.apply(
      makeEvent(0, 101, MdAction::Cancel, MdSide::Bid, UNDEF_PRICE, 2, 1, 3));
  dispatcher.apply(
  ...
  REQUIRE(dispatcher.stats().snapshots == 2);
  ...
  const auto book10 =
      std::find_if(summaries.begin(), summaries.end(),
                   [](const BookSummary &summary) {
                     return summary.instrument_id == 10;
                   });
  REQUIRE(book10 != summaries.end());
  REQUIRE(book10->best_bid->size == 3);
}

TEST_CASE("ShardedBookDispatcher flags ambiguous missing-instrument routing",
          "[lob][dispatcher][sharded]") {
  ShardedBookDispatcher dispatcher(
      DispatchOptions{}, ShardedDispatchOptions{.worker_count = 2, .queue_capacity = 4});
