...
namespace {
MarketDataEvent makeAdd(const std::string& orderId, char side, double price, long long size)
{
    MarketDataEvent e;
    e.action = 'A';
    e.side = side;
...
TEST_CASE("SimulatedLOB keeps independent engine-private historical consumption", "[SimulatedLOB]")
{
...
TEST_CASE("SimulatedLOB exposes only the current engine's synthetic orders", "[SimulatedLOB]")
{
    auto historical = std::make_shared<HistoricalLOB>();
    historical->applyEvent(makeAdd("ask-1", 'A', 101.0, 10));
    historical->applyEvent(makeAdd("bid-1", 'B', 100.0, 20));
...
TEST_CASE("Multiple EngineViews can trade concurrently on one HistoricalLOB", "[SimulatedLOB]")
{
    auto historical = std::make_shared<HistoricalLOB>();
    ...
    for (int i = 0; i < engineCount; ++i)
        engines.push_back(std::make_shared<EngineView>("engine-" + std::to_string(i)));

    for (int i = 0; i < engineCount; ++i)
    {
        threads.emplace_back([historical, view = engines[static_cast<std::size_t>(i)], i, &allThreadsOk]() {
            SimulatedLOB lob(historical, view);
            const auto result = lob.placeLimitOrder("buy-" + std::to_string(i), 'B', 101.0, 1);
