...
TEST_CASE("MarketDataEvent sameOrderKey matches the comparator", "[mde]") {
  MarketDataEvent a{};
  a.ts_recv = 42;
  a.publisher_id = 3;
  a.sequence = 7;
  // differ in fields outside the key — comparator should treat as equivalent,
  // but events are not "equal" in the full-payload sense
  MarketDataEvent b = a;
  b.order_id = 99999;
  b.price = 123;
  b.size = 5;
